...
{
	ges_exit_threshold = get_nswe_max(nswe_offset) + 5+10;
	tmg399x_set_ges_thresh(chip, prox_entry_threshold, ges_exit_threshold);
	hidden_data = true;
	mfilter_dc_count = 0;
	printk(KERN_INFO "set_hidden_data_mode ===prox_entry_threshold=%d,ges_exit_threshold=%d\n",prox_entry_threshold,ges_exit_threshold);
}
...
static void ges_output_dir4_event(void)
{
	...
	else//no used tap 
	{
		...
		else if(ges_event.dir4.enter_dir  == 6)
		{
			ges_motion &= ~0x80;
			ges_motion &=~GES_MOTION_MASK;
			ges_motion |= 2;
		...
		else if(ges_event.dir4.enter_dir == 0)
		{
			ges_motion &= ~0x80;
			ges_motion &=~GES_MOTION_MASK;
			ges_motion |= 4;
		...
		else if(ges_event.dir4.enter_dir == 2)
		{
			ges_motion &= ~0x80;
			ges_motion &=~GES_MOTION_MASK;
			ges_motion |= 6;
...
static void ges_output_event(struct tmg399x_chip *chip,struct sGesture_DS enter_dir,struct sGesture_DS exit_dir)
{
	...
	if(mrawdataarray.length >= 45)	ges_motion |=GES_SPEED_SLOW;
	else if (mrawdataarray.length >= 11)	ges_motion |=GES_SPEED_MID;
	else ges_motion |=GES_SPEED_FAST;
...
//	int ne, nw, se, sw;
	...
	vector->ne = (vector->nswe.north + vector->nswe.east) / 2;
	vector->nw = (vector->nswe.north + vector->nswe.west) / 2;
	vector->se = (vector->nswe.south + vector->nswe.east) / 2;
	vector->sw = (vector->nswe.south + vector->nswe.west) / 2;

	vector->ne_sw = vector->ne - vector->sw;
	vector->nw_se = vector->nw - vector->se;

//	printk(KERN_INFO " get_vector===>vector->index=%d\n",vector->index);
//	printk(KERN_INFO " get_vector===>n_s=%d,e_w=%d\n",vector->n_s,vector->e_w);
//	printk(KERN_INFO " get_vector===>ne=%d,nw=%d,se=%d,sw=%d\n",vector->ne,vector->nw,vector->se,vector->sw);
}
...
static void get_enter_sum_vector(struct sGesture_Diff *vector)
{
	int index;
	int north_sum = 0;
	int south_sum = 0;
	...
	for(index = 0; index < mrawdataarray.peak_nswe_index; index++)
	{
		north_sum += mrawdataarray.rawdata[index].nswe.north;
		south_sum += mrawdataarray.rawdata[index].nswe.south;
		west_sum += mrawdataarray.rawdata[index].nswe.west;
		east_sum += mrawdataarray.rawdata[index].nswe.east;
	
	}
	vector->n_s = north_sum - south_sum;
	vector->e_w = east_sum - west_sum;
}

static void get_exit_sum_vector(struct sGesture_Diff *vector)
{
	int index;
	int north_sum = 0;
	int south_sum = 0;
	...
	for(index = mrawdataarray.length-1;index>0;index--)
	{
		if(mrawdataarray.rawdata[index].cur_sum_nswe_max >= mrawdataarray.sum_nswe_max)
		{
			thres_index = index;
			break;
		}
	...
	for(index = mrawdataarray.length-1; index > thres_index; index--)
	{
		north_sum += mrawdataarray.rawdata[index].nswe.north;
		south_sum += mrawdataarray.rawdata[index].nswe.south;
		west_sum += mrawdataarray.rawdata[index].nswe.west;
...
static void create_ges_vector(int test)
{
	...
	case 1:
		/* get enter and exit vector by sum */
		get_enter_sum_vector(&ges_vector.enter);
		get_exit_sum_vector(&ges_vector.exit);
		break;
...
static void add_ges_rawdata(struct tmg399x_chip *chip, struct sRawData *prawdata)
{
	...
	prawdata->gproxmin= get_nswe_min(prawdata->nswe);
	prawdata->cur_sum_nswe_max = sum_nswe_data(prawdata->nswe);	
	
	/* check prox event here */
	if (chip->prx_enabled) {
		if (hidden_data) {
        	       	if (!prox_state && prawdata->gproxmin > PROX_THRESH_HIGH) {
        	       	        /* report prox near event here */
        	       	        prox_state = true;
				tmg399x_report_prox(chip, prox_state);
        	       	} else if (prox_state && prawdata->gproxmin < PROX_THRESH_LOW) {
	...
	if (mrawdataarray.count < array_length) {
		...
		if (mrawdataarray.sum_nswe_max < prawdata->cur_sum_nswe_max) {//get nswe max 
			mrawdataarray.sum_nswe_max = prawdata->cur_sum_nswe_max;
			mrawdataarray.peak_nswe_index = mrawdataarray.length;
		}
		
		memcpy((void*)&mrawdataarray.rawdata[mrawdataarray.length],(void*)prawdata, sizeof(struct sRawData));
		mrawdataarray.rawdata[mrawdataarray.length].cur_sum_nswe_max = prawdata->cur_sum_nswe_max;
...
static void process_ges_states(struct tmg399x_chip *chip, struct sRawData *prawdata)
{
	...
	case STARTED:
		...
		break;
	case TENDED:
		printk(KERN_INFO "mrawdataarray.length = %d\n", mrawdataarray.length);
...
static void update_stdev_filter(int data)
{
	...
	if (stdev100 != 0) {
		if(stdev100 < 200)
			stdev100 = 200;	
		stdev_filter100 = stdev100;//filter(10, stdev100, &stdev_filter100_10);
	}
}
...
void process_rgbc_data(struct tmg399x_chip *chip, struct sColor4 *data)
{
	...
	colorfilter.clear = filter(20, data->clear, &colorfilter16.clear);
	colorfilter.red = filter(20, data->red, &colorfilter16.red);
	colorfilter.green = filter(20, data->green, &colorfilter16.green);
	colorfilter.blue = filter(20, data->blue, &colorfilter16.blue);

	chip->als_inf.ir = (colorfilter.red + colorfilter.green+ colorfilter.blue - colorfilter.clear + 1) / 2;
