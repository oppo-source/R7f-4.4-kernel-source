...
static struct ps_adjust_para cust_ps_adjust_para_cm36686 = {
    ...
    .ps_thd_low_highlight = 900,
    .ps_thd_high_highlight = 1000,
    .ps_adjust_min = 0,
    .ps_adjust_max = 1200,
    .highlight_limit = 8000,
    .sampling_time = 60, //Unit:ms
    .sampling_count = 6,
    .dirty_adjust_limit = 1200,
    .dirty_adjust_low_thd = 120,
    .dirty_adjust_high_thd = 200,
};
...
static void psensor_delay_work_handler(struct work_struct *work)
{
	...
	if (ret >= 0) {
              lpi->ps_distance_last = adc_value > lpi->ps_close_thd_set ? 0 : 1;
...
static void lsensor_delay_work_handler(struct work_struct *work)
{
	...
	if (sense > 0) {
		...
		input_report_abs(lpi->ls_input_dev, ABS_MISC, cm36686_als_gain*adc_value/sense/100);
...
static int psensor_enable(struct cm36686_info *lpi)
{
	...
	if (lpi->ps_enable) {
		...
		ret = control_and_report(lpi, CONTROL_PS, 1, 1);  // modify by lauson . report abs 0->1.
#ifdef ALSPS_DYNAMIC_THRESHOLD
        {
             unsigned long delay = msecs_to_jiffies(g_ps_adjust_para->sampling_time);
             queue_delayed_work(g_ps_data->lp_wq, &g_ps_data->ps_adjust_thd_work, delay);      
        }
#endif
	}
...
static int psensor_disable(struct cm36686_info *lpi)
{
	...
	if (lpi->ps_enable == 0) {
		...
		ret = control_and_report(lpi, CONTROL_PS, 0, 0);
            ...
            ps_sum = 0;
            ps_count = 0;
             cancel_delayed_work_sync(&g_ps_data->ps_adjust_thd_work);
#endif
...
static void sample_work_func(struct work_struct *work)
{
    ...
    ret = cm36686_device_ctl(g_ps_data, 1);
    if (ret)
    	return;
    ...
    if (ret < 0)
    {
        printk("%s: write i2c error, ret=%d\n", __func__, ret);
        return;
    }
    ...
    for (i = 0; i < 10; i++)
   {
                ...
                {
                    if ((ps = cm36686_get_ps_reading(g_ps_data)) <= 0)
                    {
                        continue;
                    }
                }
    ...
    ret = cm36686_device_ctl(g_ps_data, 0);
    if (ret)
    	return;
...
static int set_ps_threshold(int ps, int crosstalk, struct ps_adjust_para *para, int state)
{
    ...
    g_ps_data->ps_away_thd_set = low_threshold;
    g_ps_data->ps_close_thd_set = high_threshold;
    ...
    printk(KERN_ERR"%s: low_thd %d high_thd %d  ps:%d crosstalk:%d  state:%d \n", __func__, low_threshold, high_threshold, ps, crosstalk, state);
    return 0;
}
...
static void alsps_dymamic_threshold(struct work_struct *work) 
{
    ...
    struct ps_adjust_para para = cust_ps_adjust_para_cm36686;
    unsigned long delay = msecs_to_jiffies(para.sampling_time);

    struct cm36686_info* ps_data = container_of((struct delayed_work *)work, struct cm36686_info, ps_adjust_thd_work);


    {

        if (ps_data->ps_enable)
        {
            ...
            if (g_is_resumed == 0)
            {
                printk("wait the cm36686 driver resume. \n");
                goto EXIT;                
            }
            
            if (get_ps_adc_value((uint16_t *)&ps) || ps<= 0)
            {
                printk("read ps raw data error \n");
                goto EXIT;
            }

            is_far = ps_data->ps_distance_last;
            printk(KERN_ERR"Prox is %s ps:%d ps_min:%d \n", is_far ? "far" : "near", ps, ps_min);
...
EXIT:
    ...
    queue_delayed_work(ps_data->lp_wq, &ps_data->ps_adjust_thd_work, delay);
    //printk(" %s exit .... \n", __func__);
    return ;
}
...
static ssize_t cm36686_prox_raw_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    //printk(KERN_EMERG"%s ps_value:%d \n", __func__, reading);
	return snprintf(buf, PAGE_SIZE, "%d\n", reading);
}
...
#ifdef ALSPS_DYNAMIC_THRESHOLD  
       g_ps_adjust_para = &cust_ps_adjust_para_cm36686;
       ps_min = g_ps_adjust_para->ps_adjust_max;
	INIT_DELAYED_WORK(&lpi->ps_adjust_thd_work, alsps_dymamic_threshold);
	...
	uint16_t param, int report)
{
  ...
  if(lpi->als_enable){
	...
	if (report) {
		...
		lpi->current_adc = adc_value;
              //printk(KERN_EMERG"%s level:%d value:%d \n", __func__, level, adc_value);
