...
#define CM36686_EX_ALSPS_DYNAMIC_THRESHOLD
#endif /*VENDOR_EDIT*/

#ifdef CM36686_EX_ALSPS_DYNAMIC_THRESHOLD
static int ps_min = 0;
static int ps_adjust_max = 1200;
static int dirty_adjust_low_thd = 120, dirty_adjust_high_thd = 200;
static int ps_thd_low_highlight = 900, ps_thd_high_highlight = 1000;
...
#endif /*CM36686_EX_ALSPS_DYNAMIC_THRESHOLD*/
...
#ifdef CM36686_EX_ALSPS_DYNAMIC_THRESHOLD    // Do not modify it as wilful, because it is used for algo.
	...
	const struct i2c_device_id *id)
{
  ...
  lpi->intr_pin = pdata->intr;
	...
	queue_delayed_work(lpi->lp_wq,&sample_ps_work, msecs_to_jiffies(1*HZ)); 
	...
	uint16_t param, int report)
{
	...
	uint16_t ps_data = 0;
	int level = 0, i, val;
	//pr_err("%s enter param:0x%x report:%d \n", __func__, param, report);
