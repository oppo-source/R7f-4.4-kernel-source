            ...
            if (doze_buf[2] != 0)
            {
                ...
                switch (doze_buf[2])
                {
                    ...
                    case CIRCLE_DETECT  :
                        ...
                        if (cross(Point_input[2], Point_input[3], Point_input[4]) > 0)
                            clockwise = 1;
                        else
                            clockwise = 0;
...
static ssize_t tp_glove_write_func(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
    ...
    {
        case 0:
            //get_tp_glove_mode(i2c_connect_client);
            GTP_DEBUG("tp_glove_func will be disable\n");
            break;
        case 1:
            //get_tp_glove_mode(i2c_connect_client);
            GTP_DEBUG("tp_glove_func will be enable\n");
            break;
        default:
...
#ifdef GT9XX_SUPPORT_APK_AUTOTEST
static int tp_glove_check_read_func(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
...
static int tp_double_write_func(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
    int ret = 0;
...
//      goto exit_free_irq;
    ...
    sprintf(fw_version, "15025%x", version_info );
