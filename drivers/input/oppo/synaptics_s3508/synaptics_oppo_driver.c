...
static void synaptics_charge_mode_enable(int enable, int is_fast_charge)
{
    	...
    	if(ts_g->loading_fw)
	{
       ...
       mutex_lock(&ts_g->input_dev->mutex);
...
END:
       mutex_unlock(&ts_g->input_dev->mutex);
	return ;
}
...
/*#ifdef VENDOR_EDIT huqiao modified for 15061 TP*/
...
static int synatpitcs_fw_update(struct device *dev, bool force)
{
	...
	if(!ts->client){
       ...
       if(is_project(OPPO_15011) )   // clear all point, avoid to dirty data.
       {
	...
	call_times++;
       ...
       if(is_project(OPPO_15011) )   // clear all point, and set charge para.
       {
            int i = 0;
            TPD_ERR("%s clear point...... \n", __func__);
            for(i = 0;i < ts->max_num;i++)
            {
#ifdef TYPE_B_PROTOCOL
                input_mt_slot(ts->input_dev, i);
		  ...
		  input_mt_report_slot_state(ts->input_dev,MT_TOOL_FINGER, 0);
            ...
            ret = i2c_smbus_write_byte_data(ts_g->client, 0xff, 0x00);
            ret = i2c_smbus_write_byte_data(ts_g->client, F01_RMI_CMD00, 0x01);// soft reset.
            msleep(150);
            
            // charge para.
            ret = i2c_smbus_write_byte_data(ts->client, 0xff, 4);
            ret = i2c_smbus_write_byte_data(ts->client, F51_CUSTOM_CTRL31&0x00ff, charge_plug_status);
            ret = i2c_smbus_write_byte_data(ts->client, 0xff, 0x0); 
				...
				const char *buf, size_t size)				
{
	...
	unsigned long val;
	int rc;
	TPD_ERR("synaptics:start update fw\n");	
	if (size > 2)
		return -EINVAL;
