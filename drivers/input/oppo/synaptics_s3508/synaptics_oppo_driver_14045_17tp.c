...
/*disable irq when read data from IC*/	
	...
	printk("\nstep 1:select report type 0x14 baseline\n");
...
TEST_WITH_CBC:
	/*****set No relax******/
	ret = synaptics_rmi4_i2c_read_byte(ts->client,F54_ANALOG_CONTROL_BASE);
	tmp_old = ret&0xff;
	printk("No relax ret = %x ,tmp_old =%x ,tmp_new = %x\n",ret,tmp_old,(tmp_old | 0x01));
	ret = synaptics_rmi4_i2c_write_byte(ts->client,F54_ANALOG_CONTROL_BASE,(tmp_old | 0x01));	
	
	if(call_flow_flag){
		ret = synaptics_rmi4_i2c_write_byte(ts->client,F54_ANALOG_DATA_BASE,0x14);//select report type 0x14
		if (ret < 0) {
			TPD_ERR("read_baseline: synaptics_rmi4_i2c_write_byte failed \n");
			goto END;
		}
		
		TPD_DEBUG("close CBC oK\n");
		ret = synaptics_rmi4_i2c_read_byte(ts->client,F54_ANALOG_CONTROL_BASE+0x27);
		tmp_old = ret&0xff;
		ret = synaptics_rmi4_i2c_write_byte(ts->client,F54_ANALOG_CONTROL_BASE+0x27,tmp_old & 0xDF);	
		...
		ret = synaptics_rmi4_i2c_write_byte(ts->client, F54_ANALOG_CONTROL_BASE+0x14, 0x01);
		ret = synaptics_rmi4_i2c_read_byte(ts->client,F54_ANALOG_CONTROL_BASE+0x17);
		tmp_old = ret&0xff;
		ret = synaptics_rmi4_i2c_write_byte(ts->client,F54_ANALOG_CONTROL_BASE+0x17,(tmp_old | 0x10));	
		ret = synaptics_rmi4_i2c_write_word(ts->client,F54_ANALOG_COMMAND_BASE,0x04);	
		checkCMD();
		prow = (uint16_t *)(fw->data + ph->array_limitcbc_offset);
		TPDTM_DMESG("array_limitcbc_offset test baseline\n");	
	}	
	...
	for(x=0;x<7;x++)
	{
		TPDTM_DMESG("Check Trx-trx trx-vdd:buf[%d]=%d\n",x,buffer[x]);
		if(buffer[x]){
			error_count++;
			goto END;	
		}
...
{ 
	...
	ret = synaptics_rmi4_i2c_write_byte(ts_g->client, F54_ANALOG_CONTROL_BASE+0x14, 0x01);
	ret = synaptics_rmi4_i2c_read_byte(ts_g->client,F54_ANALOG_CONTROL_BASE+0x17);
	tmp_old = ret&0xff;
	ret = synaptics_rmi4_i2c_write_byte(ts_g->client,F54_ANALOG_CONTROL_BASE+0x17,(tmp_old | 0x10));	
