...
#ifdef VENDOR_EDIT
/*OPPO liubin 2015-05-14 add for 14045 torch ramp time too long*/
...
static struct msm_camera_i2c_reg_array lm3642_engine_array[] = {
	{0x0A, 0x12},
	{0x08, 0x04},
	{0x09, 0x0A},
};
...
static int msm_led_cci_test_off(void)
{
	...
	if (ENGINE_MODE != led_test_mode) {
		int i = 0;
		pr_err("no-engine,lm3642 led_off set 375.74ma\n");
                        	...
                        	size_t len, loff_t *data)
{
	...
	case 9:
		mutex_lock(&flash_mode_lock);
		msm_led_cci_test_init();
		led_test_mode = ENGINE_MODE;
		mutex_unlock(&flash_mode_lock);
		msm_flash_led_engine(&fctrl);
...
static struct msm_camera_i2c_reg_setting lm3642_engine_setting = {
	.reg_setting = lm3642_engine_array,
	.size = ARRAY_SIZE(lm3642_engine_array),
	.addr_type = MSM_CAMERA_I2C_BYTE_ADDR,
	.data_type = MSM_CAMERA_I2C_BYTE_DATA,
	.delay = 0,
};
...
static struct msm_led_flash_reg_t lm3642_regs = {
	...
	.engine_setting = &lm3642_engine_setting,
};
...
static struct msm_flash_fn_t lm3642_func_tbl = {
	...
	.flash_led_engine = msm_flash_led_engine,
#endif
};
