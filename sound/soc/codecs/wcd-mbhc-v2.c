...
static bool wcd_is_special_headset(struct wcd_mbhc *mbhc)
{
	...
	while (result2 & 0x01)  {
    	#ifndef VENDOR_EDIT
    	//John.Xu@PhoneSw.AudioDriver, 2015/03/23, Modify for headset not reconiged
    	/*
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	{
	#ifndef VENDOR_EDIT
	//John.Xu@PhoneSw.AudioDriver, 2015/03/23, Modify for heaset not reconiged
	/*
		if (mbhc->hs_detect_work_stop || wcd_swch_level_remove(mbhc))
	*/
	#else /* VENDOR_EDIT */
	...
	*/
		...
		if (result2 == 1) 
		{
			...
			//Jianfeng.Qiu@PhoneSw.AudioDriver, 2015/03/25, Modify for 15011
			if((hph_count == 6) && ((is_project(OPPO_14043)) \
			    || (is_project(OPPO_14042)) || (is_project(OPPO_14041)) \
			    || (is_project(OPPO_14037)) || (is_project(OPPO_14039)) \
			    || (is_project(OPPO_14040)) || (is_project(OPPO_15005) || is_project(OPPO_15025)) \
			    || (is_project(OPPO_15011)) || (is_project(OPPO_14045)) ))
			{
...
static void wcd_mbhc_swch_irq_handler(struct wcd_mbhc *mbhc)
{
	...
	if ((mbhc->current_plug == MBHC_PLUG_TYPE_NONE) &&
		...
		if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mb_source)
			mbhc->mbhc_cb->enable_mb_source(codec, true);
			#ifdef VENDOR_EDIT
			//John.Xu@PhoneSw.AudioDriver, 2015/03/23, Add for headset not reconiged
...
//John.Xu@PhoneSw.AudioDriver, 2015/04/30, Add for 15025 headset compatible
   if(is_project(OPPO_15025)){
       if(pcb_ver() != 0 && pcb_ver() != 1){
           pr_err("%s: 15025 hw version after EVT2, set MICBIAS_EXT_BYP_CAP mode\n", __func__);
           mbhc->micbias2_cap_mode = MICBIAS_EXT_BYP_CAP;
       }
   }
#endif /* VENDOR_EDIT */
